...
pal_lines <- grep("^[ \\t]+[A-Za-z0-9_-]+:\\s*\"#[0-9A-Fa-f]{6}\"", clean_lines, value = TRUE, perl = TRUE)
pal <- setNames(
...
get_val <- function(path) {
  ...
  for (key in path) {
    ...
    matches <- grep(pattern, clean_lines[curr_idx:length(clean_lines)], perl = TRUE)
    if (length(matches) == 0) return(NULL)
    curr_idx <- curr_idx + matches[1] - 1
    indent <- indent + 2
  }
  val <- sub(paste0("^\\s*", path[length(path)], ":\\s*(.*)"), "\\1", clean_lines[curr_idx], perl = TRUE)
  val <- sub("^\"(.*)\"$", "\\1", val)
...
writeLines(
  paste0(
    ...
    "  --bs-body-color: ", fg, " !important;\n\n",
    "  --bs-primary: ", primary, " !important;\n",
    "  --bs-primary-rgb: ", paste(col2rgb(primary), collapse = ", "), " !important;\n\n",
    "  --bs-secondary: ", secondary, " !important;\n",
    "  --bs-secondary-rgb: ", paste(col2rgb(secondary), collapse = ", "), " !important;\n\n",
    "  --bs-tertiary: ", tertiary, " !important;\n",
    "  --bs-tertiary-rgb: ", paste(col2rgb(tertiary), collapse = ", "), " !important;\n\n",
    "  --bs-success: ", success, " !important;\n",
    "  --bs-success-rgb: ", paste(col2rgb(success), collapse = ", "), " !important;\n\n",
    "  --bs-info: ", info, " !important;\n",
    "  --bs-info-rgb: ", paste(col2rgb(info), collapse = ", "), " !important;\n\n",
    "  --bs-warning: ", warning, " !important;\n",
    "  --bs-warning-rgb: ", paste(col2rgb(warning), collapse = ", "), " !important;\n\n",
    "  --bs-danger: ", danger, " !important;\n",
    "  --bs-danger-rgb: ", paste(col2rgb(danger), collapse = ", "), " !important;\n\n",
    "  code, pre {\n",
