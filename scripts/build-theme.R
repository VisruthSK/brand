...
colors <- lapply(yml$color, function(x) if (is.list(x) && !is.null(x$dark)) pal[[x$dark]] else NULL)
...
writeLines(
  paste0(
    ...
    "  code, pre {\n",
    "    background-color: ", pal[[yml$typography$`monospace-inline`$`background-color`$dark]], " !important;\n",
    "  }\n",
    "}\n"
  ),
